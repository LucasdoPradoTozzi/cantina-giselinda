<x-layout>
    <div class="flex flex-col items-center p-2 m-2 bg-black">
        ...
        <p class="md-2 p-2">Total Perdido R$ {{ $waste->waste_item_sum_total_price}}</p>
        @foreach($waste->wasteItem as $item)
        <table class="min-w-full divide-y divide-gray-700 bg-black">
            <thead class="bg-gray-800">
                <tr>
                    <x-table-header class="w-1/4">Produto</x-table-header>
                    <x-table-header class="w-1/4">Quantidade Perdida</x-table-header>
                    <x-table-header class="w-1/4">Valor por item</x-table-header>
                    <x-table-header class="w-1/4">Valor total do item</x-table-header>
                </tr>
            </thead>
